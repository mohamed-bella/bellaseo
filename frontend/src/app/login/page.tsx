...
export default function LoginPage() {
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center bg-[#F9FAFB] overflow-hidden">
      ...
      {/* ── Glory Glow Background (Static) ── */}
      <div className="absolute inset-0 z-0">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-[#FF642D]/10 blur-[120px] rounded-full opacity-40" />
      </div>

      <div className="relative z-10 w-full max-w-[400px] px-6">
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[#1A1D23] shadow-2xl mb-6">
        ...
        <div className="bg-white p-8 rounded-[32px] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] border border-white">
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <Button
              type="submit"
              className={`w-full py-4 rounded-2xl font-bold transition-all shadow-lg ${isSuccess ? 'bg-[#10B981]' : 'bg-[#FF642D]'}`}
              isLoading={isLoading}
              disabled={isSuccess}
            >
        ...
        <div className="mt-10 text-center">
            <p className="text-[10px] font-bold text-[#D1D5DB] uppercase tracking-[0.2em] flex items-center justify-center gap-2">
              <Sparkles className="w-3 h-3 text-[#BCC1C4]" />
